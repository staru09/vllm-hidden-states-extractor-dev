...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Hidden States Info (GPU-Resident, Streaming)")
    print(f"{'='*60}")

    handles = kv_params.get("hidden_states_handles", [])
    if handles:
        num_steps = kv_params.get("hidden_states_num_steps", len(handles))
        print(f"  ✓ Steps captured: {num_steps}")
        print(f"  ✓ Handles:        {handles[0]}...{handles[-1]}" if len(handles) > 1
              else f"  ✓ Handle:         {handles[0]}")
        ...
        print()
        print(f"  Tensors are on GPU. To consume them:")
        ...
        print(f"        output = my_model(tensor)  # prefill: [seq_len, H], decode: [1, H]")
        print(f"")
        print(f"    # Or stacked:")
        print(f"    all_hidden, all_meta = consumer.get_all(handles)")
        print(f"    # all_hidden: [total_tokens, hidden_size]")
    else:
